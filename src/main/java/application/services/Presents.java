...
public class Presents extends TreeDecorator {
    ...
    private Shape[] createPresent(int posX, int posY, int size) {
        ...
        return new Shape[] { presentBox, presentPackage };
    }
    ...
    private void drawWithPresents(Pane panePresent) {
        for (int index = 0; index < 200; index += 40) {
            ...
            panePresent.getChildren().addAll(this.createPresent(index, 260 - size, size));
        }
    }
}
